...
public class FilterVLCController {
     ...
     *
     * @param where can be "Start" or "End"
     */
    ...
    private static final ObjectMapper MAPPER = new ObjectMapper()
            .enable(SerializationFeature.INDENT_OUTPUT); // pretty JSON in clipboard

    @FXML
    public void getCopyRaw() {
        try {
            // Map your table rows to JSON-friendly objects
            List<MediaCutData> rows = mDataTable.stream()
                    .map(t -> new MediaCutData(
                            t.getStart().toSeconds(),
                            t.getEnd().toSeconds(),
                            t.getDescription()
                    ))
    ...
    public void pasteRaw() {
        ...
        tryBlock : try {
            Clipboard clipboard = Clipboard.getSystemClipboard();
            if (!clipboard.hasString()) throw new IllegalStateException("Clipboard has no string content.");

            String json = clipboard.getString();
            if (json == null || json.isBlank()) throw new IllegalStateException("Clipboard string is empty.");

            // Parse JSON array of objects: [{start,end,description}, ...]
            List<MediaCutData> rows = MAPPER.readValue(json, new TypeReference<List<MediaCutData>>() {
            });

            if (rows.isEmpty()) throw new IllegalStateException("No rows in JSON.");

            List<FilterVLCViewModel> parsed = new ArrayList<>();
            int added = 0;
            int idx = 0;

            for (MediaCutData r : rows) {
                idx++;
                // basic validation
                if (Double.isNaN(r.getStart()) || Double.isNaN(r.getEnd())) {
                    warn.append("\n- Row ").append(idx).append(": NaN values, skipped.");
                    continue;
                }
                Duration start = Duration.seconds(r.getStart());
                Duration end = Duration.seconds(r.getEnd());
                String desc = (r.getTitle() == null) ? "" : r.getTitle();

                try {
                    if (studyConflict(start, end)) {
                        parsed.add(new FilterVLCViewModel(start, end, desc));
                        added++;
                    } else {
                        warn.append("\n- Row ").append(idx).append(": conflict, skipped.");
                    }
                } catch (Exception conflictEx) {
                    warn.append("\n- Row ").append(idx).append(": error: ").append(conflictEx.getMessage());
                }
            }

            if (added == 0) throw new IllegalStateException("No valid rows were parsed.");

            // Commit only after successful parse
            mDataTable.clear();
            mDataTable.addAll(parsed);
            Collections.sort(mDataTable); // keep your conflict algorithm assumption
            resetForm();
            reGenerateSceneNumbering();   // visual only

            DialogHelper.showAlert(
                    AlertType.INFORMATION,
                    "Paste Raw Data",
                    "Congrats! Content Parsed Successfully",
                    "Notes: - " + added + " rows parsed." + warn
            );
        ...
        if (error) {
            DialogHelper.showTextInputDialog(
                    ...
                    "Something went wrong.\nAre you sure you copied valid JSON?\n" +
